...
Liscense: MIT
********************************/
/** Jian copied and modified from https://github.com/jcampbell1/simple-file-manager 
 **/
...
$hidden_extensions = array('php' , 'php~'); // must be an array of lowercase file extensions. Extensions hidden in directory index
...
if($PASSWORD) {
	...
	if(!$_SESSION['_sfm_allowed']) {
		...
		echo '<html><body><form action=? method=post>PASSWORD(密码):<input type=password name=p autofocus/></form></body></html>';
		exit;
	}
}
...
#file_drop_target { width:500px; padding:12px 0; border: 8px dashed #ccc; font-size:12px;color:#555;
	text-align: center;float:right;margin-right:20px;}
#file_drop_target.drag_over {border: 8px dashed black; color: black; background-color: lightGreen; font-weight: 400}
...
<?php endif; ?>
	function list() {
		...
		$.get('?do=list&file='+ hashval,function(data) {
			...
			$('#table').retablesort();
            $('#maxUploadSize').text(formatFileSize(MAX_UPLOAD_SIZE));
		},'json');
	}
	function renderFileRow(data) {
		...
		var $html = $('<tr />')
			...
			.append( $('<td/>').attr('data-sort',data.mtime).text(formatTimestamp2(data.mtime)) )
    ...
    function formatTimestamp2(unix_timestamp) {
        var d = new Date(unix_timestamp*1000);
        // Someone want time to be displayed. Maybe use toISOString()?
        //return d.toLocaleDateString("en-US", {hour12: false});
        // return d.toISOString();
        return d.toLocaleDateString("en-US",{year: "numeric",month:"2-digit",day:"2-digit",hour:"2-digit", minute:"2-digit", second:"2-digit", hour12:false})
    }
...
<div id="top">
<?php if(true /*$allow_create_folder*/ ): ?>
	<form action="?" method="post" id="mkdir" />
<label for=dirname>Create New Folder(创建目录)</label><input id=dirname type=text name=name value="" />
		<input type="submit" value="create(创建)"  />
	</form>
   ...
   <?php if($allow_upload): ?>
	...
	<div id="file_drop_target">
		Drag Files Here To Upload.
            (从Windows File Explorer[文件管理器]拖放一个或多个文件到此处自动上传).
                                 <b>or (或者点击选择上传文件：)</b>
...
<table id="table"><thead><tr>
   ...
   <footer>simple php filemanager，最大文件大小上传限制:  <span id="maxUploadSize" ></span>  </footer>
</body></html>
